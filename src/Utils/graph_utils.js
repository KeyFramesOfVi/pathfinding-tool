...
const isPathObstructed = (x1, y1, x2, y2, walls) =>
  walls.some(wall =>
    getLineIntersection(x1, y1, x2, y2, wall.x1, wall.y1, wall.x2, wall.y2)
  )

const isPointInBuffer = (x, y, walls) =>
  walls.some(
    wall => x >= wall.p1x && x <= wall.p3x && y <= wall.p2y && y >= wall.p4y
  )
...
const getNodeID = (x, y, nodes) =>
  nodes.find(node => node.x === x && node.y === y).id || -1

const getCostTo = (current, neighbor) =>
  current.nodeEdges.find(
    nodeEdge =>
